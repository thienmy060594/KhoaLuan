...
namespace KiemDinhChatLuongBUS
{
    public class MonTienQuyetBUS
    {
        ...
        public List<MonTienQuyetDTO> GetListMonTienQuyet()
        {
            ...
            string query = "SELECT MTQuyet.ID_MonHoc, MTQuyet.ID_MonHoc_TienQuyet, MHoc1.MaMonHoc, MHoc1.TenMonHoc, MHoc2.MaMonHoc, MHoc2.TenMonHoc, MTQuyet.GhiChu " +
                           "FROM dbo.MonTienQuyet MTQuyet, dbo.MonHoc MHoc1, dbo.MonHoc MHoc2 " +
                           "WHERE MTQuyet.ID_MonHoc = MHoc1.ID_MonHoc AND MTQuyet.ID_MonHoc = MHoc2.ID_MonHoc";
        ...
        {
            string query = string.Format("INSERT dbo.ChuongTrinhDaoTao (ID_MonHoc, ID_MonHoc_TienQuyet, GhiChu ) VALUES (N'{0}', N'{1}', N'{2}')", id_monhoc, id_monhoc_tienquyet, ghichu);
        ...
        {
            string query = string.Format("UPDATE dbo.ChuongTrinhDaoTao SET ID_MonHoc = N'{1}', ID_MonHoc_TienQuyet = N'{2}' WHERE ID_ChuongTrinhDaoTao = N'{0}'", id_monhoc, id_monhoc_tienquyet, ghichu);
            int result = DataBaseConnection.Instance.ExcuteNonQuery(query);
            return result > 0;
        }
